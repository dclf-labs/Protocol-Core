...
const TRANSPORT_LZ = 0;
const TRANSPORT_HYPERLANE = 1;
const CHAIN_ID_SRC = 1;
const CHAIN_ID_DST = 2;
const HL_DOMAIN = 99;
const DECIMAL_CONVERSION_RATE = 10n ** 12n; // 18 local - 6 shared decimals

// Encode the minimal 40-byte OFT message: bytes32 recipient ++ uint64 amountSD
function encodeOFTMsg(recipient: string, amountLD: bigint): string {
...
describe('BridgeRateLimiterUpgradeable — USNUpgradeableHyperlane', function () {
  ...
  async function deployToken(endpoint: EndpointV2Mock): Promise<USNUpgradeableHyperlane> {
    ...
    const token = Factory.attach(await proxy.getAddress()) as unknown as USNUpgradeableHyperlane;
  ...
  beforeEach(async function () {
    ...
    // Wire LZ routing
    await endpointSrc.setDestLzEndpoint(await tokenDst.getAddress(), await endpointDst.getAddress());
    await endpointDst.setDestLzEndpoint(await tokenSrc.getAddress(), await endpointSrc.getAddress());
    await tokenSrc.setPeer(CHAIN_ID_DST, ethers.zeroPadValue(await tokenDst.getAddress(), 32));
    await tokenDst.setPeer(CHAIN_ID_SRC, ethers.zeroPadValue(await tokenSrc.getAddress(), 32));
  ...
  async function handleAs(
    ...
    await network.provider.send('hardhat_setBalance', [mailboxAddr, '0x1000000000000000000']);
  ...
  // helper: impersonate endpoint to simulate lzReceive on a token
  async function lzReceiveAs(
    ...
    await network.provider.send('hardhat_setBalance', [endpointAddress, '0x1000000000000000000']);
    const impersonated = await ethers.getImpersonatedSigner(endpointAddress);
    const origin = { srcEid, sender: ethers.zeroPadValue(peerAddress, 32), nonce: 1n };
    const guid = ethers.zeroPadValue('0x01', 32);
    const message = encodeOFTMsg(recipientAddress, amountLD);
    return token.connect(impersonated).lzReceive(origin, guid, message, ethers.ZeroAddress, '0x');
  }
    ...
    it('emits RateLimitSet per entry', async function () {
      await expect(
        tokenSrc.setRateLimits([
          { transport: TRANSPORT_HYPERLANE, remoteId: HL_DOMAIN, outbound: true, limit: LIMIT, window: WINDOW },
          { transport: TRANSPORT_LZ, remoteId: CHAIN_ID_DST, outbound: false, limit: TEN, window: WINDOW },
        ])
      )
  ...
  describe('resetInFlight', function () {
    it('reverts for non-owner', async function () {
      await expect(
        tokenSrc.connect(outsider).resetInFlight(TRANSPORT_HYPERLANE, HL_DOMAIN, true)
      ).to.be.revertedWithCustomError(tokenSrc, 'OwnableUnauthorizedAccount');
    });
    ...
    it('clears in-flight and restores full availability', async function () {
      ...
      await tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, LIMIT, { value: 0 });

      await expect(
        tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, ONE, { value: 0 })
      ...
      await expect(
        tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, LIMIT, { value: 0 })
      ).to.not.be.reverted;
    });
  });
  ...
  describe('getRateLimit', function () {
    it('returns max uint256 available when limit is 0 (unlimited)', async function () {
      const { limit, available } = await tokenSrc.getRateLimit(TRANSPORT_HYPERLANE, HL_DOMAIN, true);
    ...
    it('reflects partial consumption', async function () {
      ...
      const recipient = ethers.zeroPadValue(await other.getAddress(), 32);
      await tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, TEN, { value: 0 });

      const { available } = await tokenSrc.getRateLimit(TRANSPORT_HYPERLANE, HL_DOMAIN, true);
      expect(available).to.be.closeTo(LIMIT - TEN, ethers.parseUnits('0.01', 18));
    });
  });
  ...
  describe('Hyperlane outbound rate limit', function () {
    ...
    it('passes when amount is under limit', async function () {
      await expect(
        tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, TEN, { value: 0 })
      ).to.not.be.reverted;
    });
    ...
    it('reverts with RateLimitExceeded when amount exceeds limit', async function () {
      ...
      await expect(
        tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, LIMIT + ONE, { value: 0 })
      ).to.be.revertedWithCustomError(tokenSrc, 'RateLimitExceeded');
    });
  ...
  describe('Hyperlane inbound rate limit', function () {
    beforeEach(async function () {
      await tokenSrc.setRateLimits([
        { transport: TRANSPORT_HYPERLANE, remoteId: HL_DOMAIN, outbound: false, limit: LIMIT, window: WINDOW },
      ]);
    });
    ...
    it('passes when amount is under limit', async function () {
      ...
      expect(await tokenSrc.balanceOf(await user.getAddress())).to.equal(balBefore + TEN);
    });
    ...
    it('reverts with RateLimitExceeded when amount exceeds limit', async function () {
      ...
      expect(await tokenSrc.balanceOf(await user.getAddress())).to.equal(balBefore);
    });
  });
  ...
  describe('LZ outbound rate limit', function () {
    const lzOptions = '0x00030100110100000000000000000000000000030d40';
    ...
    it('passes when limit is 0 (unlimited)', async function () {
      ...
      const balBefore = await tokenSrc.balanceOf(await user.getAddress());
      await expect(
        tokenSrc.connect(user).send(sendParam, fee, await user.getAddress(), { value: fee.nativeFee })
      ).to.not.be.reverted;
      expect(await tokenSrc.balanceOf(await user.getAddress())).to.be.lt(balBefore);
    });

    it('reverts with RateLimitExceeded when amount exceeds limit', async function () {
      await tokenSrc.setRateLimits([
        { transport: TRANSPORT_LZ, remoteId: CHAIN_ID_DST, outbound: true, limit: TEN, window: WINDOW },
      ]);
  ...
  describe('LZ inbound rate limit', function () {
    it('passes when limit is 0 (unlimited)', async function () {
      ...
      expect(await tokenDst.balanceOf(await user.getAddress())).to.equal(balBefore + TEN);
    });

    it('reverts with RateLimitExceeded when amount exceeds limit', async function () {
      await tokenDst.setRateLimits([
        { transport: TRANSPORT_LZ, remoteId: CHAIN_ID_SRC, outbound: false, limit: TEN, window: WINDOW },
      ]);
      const balBefore = await tokenDst.balanceOf(await user.getAddress());
      await expect(
        lzReceiveAs(
      ...
      expect(await tokenDst.balanceOf(await user.getAddress())).to.equal(balBefore);
    });
  });
  ...
  describe('window decay', function () {
    ...
    it('in-flight decays proportionally with elapsed time', async function () {
      ...
      await expect(
        tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, halfLimit, { value: 0 })
      ).to.not.be.reverted;
    });
    ...
    it('available never goes below zero when decay exceeds in-flight', async function () {
      await tokenSrc.connect(user).sendTokensViaHyperlane(HL_DOMAIN, recipient, ONE, { value: 0 });
