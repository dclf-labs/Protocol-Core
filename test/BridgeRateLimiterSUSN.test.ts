...
import type { StakedUSNOFTHyperlane, EndpointV2Mock, MockMailbox } from '../typechain-types';
...
const HL_DOMAIN = 99;
const DECIMAL_CONVERSION_RATE = 10n ** 12n;
...
describe('BridgeRateLimiterUpgradeable — StakedUSNOFTHyperlane', function () {
  ...
  async function deployToken(endpoint: EndpointV2Mock): Promise<StakedUSNOFTHyperlane> {
    ...
    return Factory.attach(await proxy.getAddress()) as unknown as StakedUSNOFTHyperlane;
  }

  // Seed balance via Hyperlane handle — the only mint path on sUSN
  async function seedBalance(token: StakedUSNOFTHyperlane, recipient: string, amount: bigint) {
  ...
  beforeEach(async function () {
    ...
    await seedBalance(tokenSrc, await user.getAddress(), ethers.parseUnits('100', 18));
  });
  ...
  describe('Hyperlane inbound rate limit', function () {
    ...
    it('passes when amount is under limit', async function () {
      ...
      const mailboxImpersonated = await ethers.getImpersonatedSigner(await mockMailbox.getAddress());
      const remoteToken = ethers.zeroPadValue(await other.getAddress(), 32);
      const message = ethers.concat([
      ...
      await expect(
        tokenSrc.connect(mailboxImpersonated).handle(HL_DOMAIN, remoteToken, message)
      ).to.not.be.reverted;
